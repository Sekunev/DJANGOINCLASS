...
# ListCreateAPIView --> Department get ve create etmeye izin verir.
...
class PersonnelListCreateView(generics.ListCreateAPIView):
    ...
    # IsStafforReadOnly permission'u ile yaptığımızı create'i owerride ederek yapabiliriz.
    def create(self, request, *args, **kwargs):
        serializer = self.get_serializer(data=request.data)
        serializer.is_valid(raise_exception=True)
        if self.request.user.is_staff:  # user staff ise 
...
# RetrieveUpdateDestroyAPIView bir obje üzerinde işlem yapmaya  yarayan view
class PersonalGetUpdateDelete(generics.RetrieveUpdateDestroyAPIView):
    ...
    def delete(self, request, *args, **kwargs):
        if request.user.is_superuser: # delete işlemini sadece superuser yapabilir.
...
class DepartmentPersonnelView(generics.ListAPIView):
    ...
    def get_queryset(self):
        ...
        return Department.objects.filter(name__iexact=name)  # name__iexact Büyük/küçük harf farketmez.
